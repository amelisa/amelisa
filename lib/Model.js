...
import { isServer, clone } from './util';
...
class Model extends EventEmitter {
  ...
  subscribe(queries) {
    return new Subscription(queries, this.collectionSet, this.querySet);
  }
  ...
  add(collectionName, docData, callback) {
    ...
    let collection = this.collectionSet.getOrCreateCollection(collectionName);
    collection.add(docId, docData, callback);
    return docId;
  }
  ...
  set(collectionName, docId, field, value, callback) {
    ...
    if (!doc) {
      if (callback) {
        callback(new Error('Fetch doc before mutate: ' +
          collectionName + ' ' + docId));
      } else {
        console.error('Fetch doc before mutate: ' +
          collectionName + ' ' + docId);
      }
      return;
    }
