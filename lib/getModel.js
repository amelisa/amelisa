...
function getModel() {
  ...
  util
    ...
    .then((storage) => {
      ...
      let { projectionHashs } = model.options;
      let prevProjectionHashes = localStorage.getProjectionHashes();

      let promises = [];

      for (let collectionName in prevProjectionHashes) {
        let prevHash = prevProjectionHashes[collectionName];
        let hash = projectionHashs[collectionName];

        if (!hash || prevHash !== hash) {
          console.log('CLEAE', collectionName)
    ...
    .then(() => {
      ...
      localStorage.setProjectionHashes(projectionHashs);
    })
    .then(() => model.init())
    .then(() => {
      model.unbundleData();
